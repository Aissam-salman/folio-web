...
export class ThinFilmFresnelMap extends THREE.DataTexture {
  ...
  constructor(
    filmThickness: number = 380,
    refractiveIndexFilm: number = 4,
    refractiveIndexBase: number = 3,
    size: number = 64
  ) {
    const _data = new Uint8Array(size * 4)
    super(
      _data,
      size,
      1,
    ...
    this.updateSettings(filmThickness, refractiveIndexFilm, refractiveIndexBase)
  }
  ...
  private _fresnelRefl(
    ...
    if (refrRatio * refrRatio * sin1Sqr > 1.0) {
      ...
      phi.set(
        2.0 * Math.atan((-sqrRefrRatio * Math.sqrt(sin1Sqr - 1.0 / sqrRefrRatio)) / cos1),
        2.0 * Math.atan(-Math.sqrt(sin1Sqr - 1.0 / sqrRefrRatio) / cos1)
      )
    } else {
      ...
      phi.set(r_p < 0.0 ? Math.PI : 0.0, r_s < 0.0 ? Math.PI : 0.0)
      R.set(r_p * r_p, r_s * r_s)
    }
  }
  ...
  private _updateData() {
    ...
    for (let i = 0; i < size; ++i) {
      ...
      R_star.set(
        (T12.x * T12.x * R23.x) / (1.0 - R23.x * R12.x),
        (T12.y * T12.y * R23.y) / (1.0 - R23.y * R12.y)
      )
...
function xFit_1931(lambda: number): number {
